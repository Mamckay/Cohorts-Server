...
router.get("/location/:range/:lat/:lng", jwtAuth, (req, res, next) => {
    ...
    /*            */
    Group.find() // maybe query by some small range of lat and lng to condense filter by general area
        .then(orgs => {
            return Promise.all(
                orgs.map(grp => {
                    let destinations = `${grp.geoLocation.lat},${
                        grp.geoLocation.lng
                        }`;
                ...
                .then(orgs => {
                    let filtered = orgs.filter(grp => grp != null);
...
router.put("/", jwtAuth, (req, res, next) => {
    ...
    Group.findOneAndUpdate({ _id: organizationId }, { organization })
        .sort({ createdAt: "desc" })
        .then(organization => {
